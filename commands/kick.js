...
module.exports = {
	...
	execute(client, message, args) {
		...
		if(!message.member.hasPermission('KICK_MEMBERS')) {
			return message.channel.send(noUserPerms).then(msg => {
				msg.delete({ timeout: 6000 });
			});
		}
		if(!message.guild.me.hasPermission('KICK_MEMBERS')) {
			return message.channel.send(noBotPerms).then(msg => {
				msg.delete({ timeout: 6000 });
			});
		}

		if (!isNaN(args[0])) {
			return message.channel.send(usedANumber).then(msg => {
				msg.delete({ timeout: 6000 });
			});
		}

		if(!member) {
			return message.channel.send(notInTheServer).then(msg => {
				msg.delete({ timeout: 6000 });
			});
		}

		if(member.id === message.author.id) {
			return message.channel.send(kickYourself).then(msg => {
				msg.delete({ timeout: 6000 });
			});
		}

		if(!member.kickable) {
			return message.channel.send(userHigherThanBot).then(msg => {
				msg.delete({ timeout: 6000 });
			});
		}
